...
import { Loader2, DollarSign, Plus, Trash2, TrendingUp, TrendingDown } from "lucide-react";
import { apiRequest, queryClient } from "@/lib/queryClient";
import { useToast } from "@/hooks/use-toast";
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from "recharts";

const EXPENSE_CATEGORIES = ["Food", "Transport", "Utilities", "Entertainment", "Medical", "Business", "Other"];
...
export default function Finance() {
  ...
  const monthlyTx = transactions.filter((t: any) => t.date?.startsWith(currentMonth));
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6" data-testid="page-finance">
      ...
      <Card data-testid="card-add-transaction">
        ...
        <CardContent>
          ...
          <Button className="mt-3" onClick={() => { if (parseFloat(amount) > 0) addMutation.mutate({ amount: parseFloat(amount), type, category, description, date }); }} disabled={addMutation.isPending} data-testid="button-add-transaction">
      ...
      <Card data-testid="card-transactions">
        ...
        <CardContent>
          {isLoading ? <Loader2 className="h-6 w-6 animate-spin mx-auto" /> : transactions.length === 0 ? <p className="text-sm text-muted-foreground">No transactions yet</p> : (
