...
const SETUP_STEPS: SetupStep[] = [
  {
    ...
    commands: {
      ...
      windows: [
        "# IMPORTANT: The curl command below does NOT work in PowerShell.",
        "# You must use WSL2 (Windows Subsystem for Linux) instead.",
        "",
        "# Step A: Open PowerShell as Administrator and install WSL2:",
        "wsl --install",
        "",
        "# Step B: Restart your computer, then open Ubuntu from the Start Menu.",
        "",
        "# Step C: Inside the Ubuntu terminal, run:",
        "curl -fsSL https://openclaw.ai/install.sh | bash -s -- --no-onboard",
      ],
    },
    tip: "Windows users must use WSL2 — the install script is a Linux bash script and won't work in PowerShell or Command Prompt. After running 'wsl --install' and restarting, open Ubuntu from the Start Menu.",
  },
  {
    title: "Set Gateway Token",
    description: "Set the gateway authentication token so the node can connect to your primary gateway.",
    icon: Shield,
    commands: {
      linux: [
        "# Find your gateway token on the primary machine:",
        "# It's in ~/.openclaw/openclaw.json under gateway.auth.token",
        "",
        "# Then set it on this node machine:",
    ...
    tip: "The gateway token is generated during the primary machine's onboarding. You only need to copy it once.",
  },
  {
    title: "Start the Node",
    description: "Connect this machine to your primary gateway as a node. Replace <gateway-ip> with the IP address of your primary OpenClaw machine.",
    icon: Wifi,
    commands: {
      linux: [
        "# Start as a foreground process (for testing):",
        "openclaw node run --host <gateway-ip> --port 18789 --display-name \"My Node\"",
        "",
        "# Or install as a background service (recommended):",
        "openclaw node install --host <gateway-ip> --port 18789 --display-name \"My Node\"",
    ...
    tip: "Replace <gateway-ip> with the IP of your primary machine (e.g. 192.168.1.100). If your gateway binds to loopback only, you may need an SSH tunnel — see the OpenClaw docs.",
  },
  {
    title: "Approve the Node",
    description: "Approve the node from the primary machine or this dashboard.",
    icon: Cpu,
    commands: {
      linux: [
        "# From the primary machine CLI:",
        "openclaw nodes pending",
        "openclaw nodes approve <requestId>",
        "",
        "# Or approve from this dashboard:",
        "# Go to Settings > Nodes and click 'Approve'",
      ],
      "linux-rhel": [
        "# From the primary machine CLI:",
        "openclaw nodes pending",
    ...
    tip: "Once approved, the node will show as 'paired' and can execute commands sent from the gateway.",
  },
  {
    title: "Verify Connection",
    description: "Check that the node is connected and working.",
    icon: Terminal,
    commands: {
      linux: [
    ...
    tip: "The node should show as 'paired' with its display name. You can also check node status from the Nodes page in this dashboard.",
  },
];
