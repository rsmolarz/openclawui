...
- **Feature Specifications**:
    ...
    - **Replit Projects**: Monitoring of Replit projects with Quick Import (text-based: names, URLs, deployment URLs), bulk JSON import, deployment health checks, Scan Deployments (probes `.replit.app` domains to auto-discover live projects), Sync All/Refresh Deployments (probes all existing projects for deployment URLs and health status), and status filtering. Includes AI-powered prioritization, Omi Insights integration, App Workbench with 3 view modes (App — embedded deployed app iframe, Editor — embedded Replit code editor via `?embed=true`, Split — side-by-side editor + app), and Cross-App Orchestrator (multi-project AI prompt interface that generates per-project implementation plans). Note: Replit GraphQL profile sync is limited due to persisted query hash requirements; Sync All provides the primary discovery mechanism.
