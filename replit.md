...
- **WhatsApp Status**: Dashboard card queries both gateway health API and standalone bot status (`/api/whatsapp/status`) for accurate status. Bot runs at `/root/openclaw-whatsapp-bot/` on VPS, connected phone: 13405140344, runtime: home-bot on PodcastPC.
