...
echo "=== DONE ===" ; echo "---SKILLS---" ; openclaw skills list 2>&1 | head -5`,
  ...
  "set-companion-url": `sed -i '/^export COMPANION_URL=/d' /root/.bashrc 2>/dev/null; echo 'export COMPANION_URL=http://192.168.0.189:8000' >> /root/.bashrc && echo "COMPANION_URL set to http://192.168.0.189:8000" && echo "Testing connectivity..." && curl -sf -m 5 http://192.168.0.189:8000/api/ 2>&1 || echo "Companion not reachable from VPS (local network only - needs Tailscale or tunnel)"`,
