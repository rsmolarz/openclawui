...
echo "=== DONE ===" ; echo "---SKILLS---" ; openclaw skills list 2>&1 | head -5`,
  ...
  "set-env-key": "echo 'Provide key and value in request body'",
  "tailscale-peers": `tailscale status 2>&1 | head -30`,
  "set-companion-url": `sed -i '/^export COMPANION_URL=/d; /^export STREAMDECK_URL=/d' /root/.bashrc 2>/dev/null; echo 'export COMPANION_URL=http://100.72.2.3:8000' >> /root/.bashrc && echo 'export STREAMDECK_URL=http://100.72.2.3' >> /root/.bashrc && echo "COMPANION_URL set to http://100.72.2.3:8000 (Mac Mini via Tailscale)" && echo "STREAMDECK_URL set to http://100.72.2.3 (Mac Mini via Tailscale)" && echo "Testing Companion connectivity..." && curl -sf -m 5 http://100.72.2.3:8000/ 2>&1 && echo "Companion reachable!" || echo "Companion not responding on port 8000 (may need to verify port)"`,
  "set-ha-credentials": `HA_TK="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJlOGY5OWZlNGUxY2M0N2ExYTQzNDllOTI3ZmUxYTEzMiIsImlhdCI6MTc3MjMyMzA3MSwiZXhwIjoyMDg3NjgzMDcxfQ.txLvSV8fh14sMYERn4jhtGscc8LPPsdD2nFucGqgWJU" && HA_U="https://7dh9qbje0m4st3shm4vmud3hxiikcpxp.ui.nabu.casa" && sed -i '/^export HA_URL=/d; /^export HA_TOKEN=/d' /root/.bashrc 2>/dev/null; echo "export HA_URL=$HA_U" >> /root/.bashrc && echo "export HA_TOKEN=$HA_TK" >> /root/.bashrc && echo "HA credentials written to .bashrc" && echo "HA_URL=$HA_U" && echo "HA_TOKEN length: $(echo -n $HA_TK | wc -c)"`,
...
for ENTRY in $VNODES; do
  ...
  "kill-virtual-nodes": "for f in /etc/systemd/system/openclaw-vnode-*.service; do rm -f $f; done; systemctl daemon-reload; pkill -9 -f 'openclaw node run' 2>/dev/null; sleep 1; pkill -9 -f 'openclaw node run' 2>/dev/null; echo done; ps aux | grep 'openclaw node' | grep -v grep | wc -l",
