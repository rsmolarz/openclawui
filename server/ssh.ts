...
const ALLOWED_COMMANDS: Record<string, string> = {
  ...
  "cli-devices-list": `TOKEN=$(python3 << 'PYEOF'
import json
with open("/root/.openclaw/openclaw.json") as f:
    d = json.load(f)
print(d["gateway"]["auth"]["token"])
PYEOF
) && openclaw devices list --url ws://127.0.0.1:18789 --token "$TOKEN" --json 2>&1 || echo '{"error":"command failed"}'`,
  "cli-nodes-status": `TOKEN=$(python3 << 'PYEOF'
import json
with open("/root/.openclaw/openclaw.json") as f:
    d = json.load(f)
print(d["gateway"]["auth"]["token"])
PYEOF
) && openclaw nodes status --url ws://127.0.0.1:18789 --token "$TOKEN" --json 2>&1 || echo '{"error":"command failed"}'`,
  "cli-nodes-pending": `TOKEN=$(python3 << 'PYEOF'
import json
with open("/root/.openclaw/openclaw.json") as f:
    d = json.load(f)
print(d["gateway"]["auth"]["token"])
PYEOF
) && openclaw nodes pending --url ws://127.0.0.1:18789 --token "$TOKEN" --json 2>&1 || echo '{"error":"command failed"}'`,
  "gateway-call-health": `TOKEN=$(python3 -c "import json; d=json.load(open('/root/.openclaw/openclaw.json')); print(d['gateway']['auth']['token'])") && openclaw gateway call health --url ws://127.0.0.1:18789 --token "$TOKEN" --json --timeout 15000 2>&1 || echo '{"error":"gateway call health failed"}'`,
  "gateway-call-status": `TOKEN=$(python3 -c "import json; d=json.load(open('/root/.openclaw/openclaw.json')); print(d['gateway']['auth']['token'])") && openclaw gateway call status --url ws://127.0.0.1:18789 --token "$TOKEN" --json --timeout 15000 2>&1 || echo '{"error":"gateway call status failed"}'`,
