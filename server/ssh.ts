...
echo "=== DONE ===" ; echo "---SKILLS---" ; openclaw skills list 2>&1 | head -5`,
  ...
  "set-ha-credentials": `HA_TK="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJlOGY5OWZlNGUxY2M0N2ExYTQzNDllOTI3ZmUxYTEzMiIsImlhdCI6MTc3MjMyMzA3MSwiZXhwIjoyMDg3NjgzMDcxfQ.txLvSV8fh14sMYERn4jhtGscc8LPPsdD2nFucGqgWJU" && HA_U="http://homeassistant.local:8123" && sed -i '/^export HA_URL=/d; /^export HA_TOKEN=/d' /root/.bashrc 2>/dev/null; echo "export HA_URL=$HA_U" >> /root/.bashrc && echo "export HA_TOKEN=$HA_TK" >> /root/.bashrc && echo "HA credentials written to .bashrc" && echo "HA_URL=$HA_U" && echo "HA_TOKEN length: $(echo -n $HA_TK | wc -c)"`,
