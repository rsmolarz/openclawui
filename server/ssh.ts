...
for ENTRY in $VNODES; do
  ...
  "kill-virtual-nodes": "ls /etc/systemd/system/openclaw-vnode-* 2>/dev/null && rm -f /etc/systemd/system/openclaw-vnode-*.service || true; systemctl daemon-reload; kill -9 2227542 2>/dev/null; echo 'done'; ps aux | grep openclaw-node | grep -v grep",
